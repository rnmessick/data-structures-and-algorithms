...
const hasChildrenValues = (arr, character) => {
  ...
  for(let i = 0; i < arr.length; i++){
    let allCharacterValues = Object.values(arr[i]);
    if(allCharacterValues[0] === character){
      if(allCharacterValues[2].length > 0){
        hasKids = true;
      }
    }
  }
...
const hasChildrenEntries = (arr, character) => {
  ...
  let hasKids = false;
  for( let i =0; i < arr.length; i++){
    let allCharacterEntries = Object.entries(arr[i]);
    if(allCharacterEntries[0][1] === character){
      if(allCharacterEntries[2][1].length > 0){
        hasKids = true;
      }
    }
  }
