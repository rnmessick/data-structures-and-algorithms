...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let regexSplit = /\(|\)|\s|-/;
  return arr.map(string => string.split(regexSplit).join(''));
...
const onlyOddChars = (str) => {
  ...
  let filteredArray = stringArray.filter((element, index) =>{
    if (index % 2 === 1){
      return true;
    }
  })
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(studentRosterArray => studentRosterArray.filter(student =>{
